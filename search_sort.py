...
def sort_title_asc(playlist):
    ...
    while swapped:  # Loop sampai tidak ada pertukaran lagi
        ...
        while curr and curr.next:  # Loop sampai sebelum tail (dan curr tidak None)
            if curr.title > curr.next.title:  # Jika judul saat ini > berikutnya, tukar
                # Simpan next_node karena akan diubah
                next_node = curr.next
                
                # Proses pertukaran node di doubly linked list
                prev = curr.prev
                if prev:
                    prev.next = next_node
                else:
                ...
                # Setelah swap, curr sekarang di belakang next_node
...
def sort_duration_asc(playlist):
    ...
    """
    # Helper function untuk konversi durasi ke detik
    def duration_to_sec(dur):
        try:
            # Handle format MM.SS atau MM:SS
            if '.' in dur:
    ...
    while swapped:  # Loop bubble sort
        ...
        while curr and curr.next:  # Loop sampai sebelum tail (dan curr tidak None)
            d1 = duration_to_sec(curr.duration)
            d2 = duration_to_sec(curr.next.duration)
